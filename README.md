...
- Fastify API with server-side price calculation, rate limiting, validation, Supabase persistence when configured, and SMTP email provider.
...
The web app intentionally does not pretend that email or database persistence exists when no API is configured; checkout displays a clear configuration error instead.
