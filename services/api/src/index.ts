...
app.get('/api/admin/overview', async (request, reply) => { try { await assertAdmin(request.headers.authorization); return reply.send(await getAdminOverview()); } catch (error) { return reply.code(401).send({ message: error instanceof Error ? error.message : 'KhÃ´ng thá»ƒ táº£i tá»•ng quan.' }); } });
app.get('/api/admin/geometry', async (request, reply) => { try { await assertAdmin(request.headers.authorization); return reply.send(await getAdminGeometry()); } catch (error) { return reply.code(401).send({ message: error instanceof Error ? error.message : 'KhÃ´ng thá»ƒ táº£i geometry projects.' }); } });
app.get('/api/admin/settings', async (request, reply) => { try { await assertAdmin(request.headers.authorization); return reply.send(await getAdminSettings()); } catch (error) { return reply.code(401).send({ message: error instanceof Error ? error.message : 'KhÃ´ng thá»ƒ táº£i settings.' }); } });
app.patch('/api/admin/settings', async (request, reply) => { try { await assertAdmin(request.headers.authorization); return reply.send(await updateAdminSettings((request.body ?? {}) as Record<string, unknown>)); } catch (error) { return reply.code(401).send({ message: error instanceof Error ? error.message : 'KhÃ´ng thá»ƒ cáº­p nháº­t settings.' }); } });
