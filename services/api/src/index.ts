...
import { createOrder, getStoreMode, listOrders } from './order.js';
...
app.get('/api/admin/orders', async (_request, reply) => { try { return reply.send(await listOrders()); } catch (error) { return reply.code(500).send({ message: error instanceof Error ? error.message : 'Không thể tải đơn hàng.' }); } });
