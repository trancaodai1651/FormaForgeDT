...
export function StlCutterPage() {
  ...
  return <main className="stl-cutter-page">
    <header className="stl-cutter-header"><div className="stl-cutter-brand"><span className="stl-cutter-mark"><Scissors size={18} /></span><h1>FormaForge<span>DT</span></h1><Link className="stl-dashboard-pill" to="/admin"><ShieldCheck size={14} />{t('admin.backToDashboard')}</Link></div><div className="stl-cutter-header-actions"><span><ShieldCheck size={14} />PUBLIC TOOL</span><button type="button" onClick={toggleLanguage}>{language === 'vi' ? 'EN' : 'VI'}</button><Link to="/admin" aria-label={t('admin.backToDashboard')}><ChevronDown size={15} /></Link></div></header>
    <div className="stl-cutter-layout">
      <aside className="stl-cutter-sidebar">
        ...
        <footer className="stl-sidebar-footer"><span><ShieldCheck size={12} />PUBLIC TOOL</span><span>{t('stlCutter.offline')}</span></footer>
      </aside>
