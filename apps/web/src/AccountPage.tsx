...
function getDisplayName(user: import('@supabase/supabase-js').User) {
  ...
  if (typeof displayName === 'string' && displayName.trim() && !displayName.includes('?')) return displayName;
...
export function AccountPage() {
  ...
  return <div className="container page-section account-page"><div className="account-header"><div><span className="eyebrow">{t('account.eyebrow')}</span><h1>{getDisplayName(user)}</h1><p>{user.email}</p></div><button className="account-logout-button" type="button" onClick={() => { void signOutAdmin().then(() => setUser(null)); }}><span className="account-logout-icon"><LogOut size={16} /></span><span>{t('auth.logout')}</span><ArrowUpRight className="account-logout-arrow" size={15} /></button></div><section className="account-card"><div className="account-card-heading"><div><span className="eyebrow">{t('account.addressTitle')}</span><h2>{t('account.addressTitle')}</h2></div><UserRound size={22} /></div><p className="muted">{t('account.addressHint')}</p><form className="account-address-form" onSubmit={submitAddress}><label>{t('account.recipient')}<input required minLength={2} value={address.recipient_name} onChange={(event) => updateAddress('recipient_name', event.target.value)} /></label><label>{t('account.email')}<input required type="email" value={address.email} onChange={(event) => updateAddress('email', event.target.value)} /></label><label>{t('account.phone')}<input required value={address.phone} onChange={(event) => updateAddress('phone', event.target.value)} /></label><label className="wide">{t('account.address')}<textarea required minLength={5} value={address.address} onChange={(event) => updateAddress('address', event.target.value)} /></label>{error && <div className="error-box wide">{error}</div>}{message && <div className="success-box wide">{message}</div>}<GlassButton disabled={saving}>{saving ? t('account.saving') : t('account.save')} <Save size={15} /></GlassButton></form></section></div>;
}
