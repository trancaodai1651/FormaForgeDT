...
export function Footer() { const { t } = useI18n(); return <footer className="site-footer"><div><span className="eyebrow">HOMETOWN MODULAR LAMP</span><p>{t('footer.tagline')}</p></div><div className="footer-links"><Link to="/about">{t('footer.story')}</Link><Link to="/contact">{t('nav.contact')}</Link><Link to="/downloads">{t('nav.downloads')}</Link><Link to="/admin">{t('footer.studio')}</Link></div><span className="muted">© 2026 Hometown Lamp</span></footer>; }
