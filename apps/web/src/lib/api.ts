...
export function inspectPriceReaderUrl(url: string, accessToken: string) { return request<PriceReaderProduct>('/admin/price-reader/inspect', { method: 'POST', headers: { 'content-type': 'application/json', authorization: `Bearer ${accessToken}` }, body: JSON.stringify({ url }) }); }
export function listPriceReaderProducts(accessToken: string) { return request<PriceReaderProduct[]>('/admin/price-reader/products', { headers: { authorization: `Bearer ${accessToken}` } }); }
export function savePriceReaderProduct(url: string, accessToken: string) { return request<PriceReaderProduct>('/admin/price-reader/products', { method: 'POST', headers: { 'content-type': 'application/json', authorization: `Bearer ${accessToken}` }, body: JSON.stringify({ url }) }); }
export function refreshPriceReaderProduct(id: string, accessToken: string) { return request<PriceReaderProduct>(`/admin/price-reader/products/${encodeURIComponent(id)}/refresh`, { method: 'POST', headers: { authorization: `Bearer ${accessToken}` } }); }
export function deletePriceReaderProduct(id: string, accessToken: string) { return request<void>(`/admin/price-reader/products/${encodeURIComponent(id)}`, { method: 'DELETE', headers: { authorization: `Bearer ${accessToken}` } }); }
