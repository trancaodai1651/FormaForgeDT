...
export function inspectPriceReaderUrl(url: string, accessToken?: string) { return request<PriceReaderProduct>('/price-reader/inspect', { method: 'POST', headers: { 'content-type': 'application/json', ...(accessToken ? { authorization: `Bearer ${accessToken}` } : {}) }, body: JSON.stringify({ url }) }); }
