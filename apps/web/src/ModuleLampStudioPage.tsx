import {
  ...
  Save, ShieldCheck, SlidersHorizontal, Sparkles, Trash2,
} from 'lucide-react';
...
import {
  HARDWARE_CATALOG, JOINT_CATALOG, SKETCH_PRESETS, createDefaultModuleProject,
  createLampModule, parseModuleProject, type HardwareId, type JointType, type LampModule,
...
import {
  ...
  getAssemblyDimensions, normalizeSketch, type ModuleGeometryPart,
...
function loadInitialProject() {
  try {
    const stored = localStorage.getItem('hometown-module-studio');
...
function SketchPad({ points, onChange }: { points: SketchPoint[]; onChange: (points: SketchPoint[]) => void }) {
  const { t } = useI18n();
  const [drawing, setDrawing] = useState(false);
  const [draft, setDraft] = useState(points);
  useEffect(() => { if (!drawing) setDraft(points); }, [drawing, points]);
  const pointFromEvent = (event: ReactPointerEvent<SVGSVGElement>): SketchPoint => {
    const bounds = event.currentTarget.getBoundingClientRect();
    const radius = Math.min(1, Math.max(.12, (event.clientX - bounds.left - 28) / Math.max(1, bounds.width - 46)));
    const height = 1 - Math.min(1, Math.max(0, (event.clientY - bounds.top - 16) / Math.max(1, bounds.height - 32)));
    return { radius, height };
  };
  const begin = (event: ReactPointerEvent<SVGSVGElement>) => {
    event.currentTarget.setPointerCapture(event.pointerId);
    const next = [pointFromEvent(event)];
    setDrawing(true); setDraft(next); onChange(next);
  };
  const move = (event: ReactPointerEvent<SVGSVGElement>) => {
    if (!drawing) return;
    const point = pointFromEvent(event);
    const previous = draft.at(-1);
    if (previous && Math.hypot(previous.radius - point.radius, previous.height - point.height) < .025) return;
  ...
  const finish = (event: ReactPointerEvent<SVGSVGElement>) => {
    ...
    const next = normalizeSketch(draft);
    setDrawing(false); setDraft(next); onChange(next);
  };
  const display = drawing ? draft : points;
  const path = display.map((point, index) => `${index ? 'L' : 'M'} ${28 + point.radius * 226} ${224 - point.height * 192}`).join(' ');
  return <div className="module-sketch-card">
    <div className="module-section-title"><div><span>01 / PROFILE</span><strong>{t('moduleStudio.sketch')}</strong></div><span className="module-live"><i />LIVE</span></div>
    <svg className="module-sketch-pad" viewBox="0 0 272 240" role="img" aria-label={t('moduleStudio.sketchHint')} onPointerDown={begin} onPointerMove={move} onPointerUp={finish} onPointerCancel={finish}>
      ...
      {display.map((point, index) => <circle key={`${point.height}-${index}`} cx={28 + point.radius * 226} cy={224 - point.height * 192} r="3.5" />)}
    </svg>
    <p>{t('moduleStudio.sketchHint')}</p>
    <div className="module-preset-row">{(['soft', 'tower', 'wave', 'bell'] as const).map((preset) => <button type="button" key={preset} onClick={() => onChange(SKETCH_PRESETS[preset].map((point) => ({ ...point })))}>{t(`moduleStudio.preset.${preset}`)}</button>)}</div>
  </div>;
}
...
function ModuleMesh({ module, y, sketch, hardware, selected, lightOn, brightness, lightColor, onSelect, onOffset }: { module: LampModule; y: number; sketch: SketchPoint[]; hardware: HardwareId; selected: boolean; lightOn: boolean; brightness: number; lightColor: string; onSelect: () => void; onOffset: (offset: number) => void }) {
  ...
  return <group position={[0, y, 0]} rotation={[0, module.rotation * Math.PI / 180, 0]} onPointerDown={beginDrag} onPointerMove={moveDrag} onPointerUp={endDrag} onPointerCancel={endDrag}>
    ...
    {selected && <mesh><cylinderGeometry args={[module.diameter / 2 + 4, module.diameter / 2 + 4, module.height + 8, 48, 1, true]} /><meshBasicMaterial color="#f0b967" wireframe transparent opacity={.32} /></mesh>}
  </group>;
}
...
function ModuleScene({ project, selectedId, cameraPreset, autoRotate, onSelect, onOffset }: { project: ModuleStudioProject; selectedId: string; cameraPreset: CameraPreset; autoRotate: boolean; onSelect: (id: string) => void; onOffset: (id: string, offset: number) => void }) {
  ...
  return <>
    ...
    {project.lightOn && <pointLight position={[0, 42, 0]} color={lightColor} intensity={project.brightness * 340} distance={420} decay={2} />}
...
function ModuleList({ modules, selectedId, onSelect, onMove, onToggle, onDropModule }: { modules: LampModule[]; selectedId: string; onSelect: (id: string) => void; onMove: (id: string, direction: -1 | 1) => void; onToggle: (id: string) => void; onDropModule: (sourceId: string, targetId: string) => void }) {
  const { t } = useI18n();
...
export function ModuleLampStudioPage() {
  ...
  const [project, setProject] = useState<ModuleStudioProject>(loadInitialProject);
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => localStorage.setItem('hometown-module-studio', JSON.stringify({ ...project, updatedAt: new Date().toISOString() })), 180);
  ...
  const removeSelected = () => {
    if (!selected || selected.kind === 'core') return;
    const modules = project.modules.filter((module) => module.id !== selected.id); updateProject({ modules }); setSelectedId(modules.at(-1)?.id ?? '');
  };
  ...
  return <main className="module-studio" data-mobile-panel={mobilePanel}>
    ...
    <div className="module-studio-layout">
      <aside className="module-studio-sidebar">
        <SketchPad points={project.sketch} onChange={(sketch) => updateProject({ sketch })} />
        <section className="module-library"><div className="module-section-title"><div><span>02 / {t('moduleStudio.library').toUpperCase()}</span><strong>{t('moduleStudio.addModule')}</strong></div></div><div className="module-library-grid">{(['sketch', 'spacer', 'diffuser', 'cap'] as ModuleKind[]).map((kind) => <button type="button" key={kind} onClick={() => addModule(kind)}><Plus size={15} /><span>{t(`moduleStudio.kind.${kind}`)}</span></button>)}</div></section>
        <section className="module-stack"><div className="module-section-title"><div><span>03 / {t('moduleStudio.assembly').toUpperCase()}</span><strong>{t('moduleStudio.stack')}</strong></div><Move3D size={16} /></div><ModuleList modules={project.modules} selectedId={selectedId} onSelect={setSelectedId} onMove={moveModule} onToggle={(id) => updateModule(id, { visible: !project.modules.find((module) => module.id === id)?.visible })} onDropModule={reorderModules} /></section>
      </aside>
